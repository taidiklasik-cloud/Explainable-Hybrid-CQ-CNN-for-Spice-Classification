...
def reset_stage_from_scratch(
    *,
    ...
    """Reset a stage completely if it has not been completed.
    
    This will delete all traces of the task in the database,
    delete the Optuna study, and set the stage back to inactive/pending.
    """
    import optuna

    signal = db.get_worker_stage_signal(stage_no, model_type)
    if signal.get("signal") == "STAGE_COMPLETE":
        raise ValueError(f"Cannot restart stage {stage_no} for {model_type} because it is already marked STAGE_COMPLETE.")
    
    stage_info = db.get_stage_info(stage_no, model_type)
    study_name = stage_info.get("optuna_study_name")
    
    print(f"[orchestrator] Purging PostgreSQL tasks for stage {stage_no} ({model_type})...")
    db.reset_stage_tasks(stage_no, model_type)
    
    if study_name:
        print(f"[orchestrator] Deleting Optuna study '{study_name}'...")
        try:
            optuna.delete_study(study_name=study_name, storage=optuna_storage_url)
            print(f"[orchestrator] Study '{study_name}' deleted.")
        except KeyError:
            print(f"[orchestrator] Study '{study_name}' not found in Optuna, skipping.")
        except Exception as e:
            print(f"[orchestrator] Warning: could not delete Optuna study '{study_name}': {e}")
    
    print(f"[orchestrator] Restart preparation complete for stage {stage_no} ({model_type}).")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Optuna Stage Manager")
    parser.add_argument("--stage", type=int, required=True, help="Stage number to manage")
    parser.add_argument("--model", type=str, required=True, help="Model type (e.g. classical_fully_spatial)")
    parser.add_argument("--restart-from-scratch", action="store_true", help="Purge all tasks and restart study")
    
    args = parser.parse_args()
    
    load_dotenv()
    db_dsn = os.environ.get("ORCHESTRATION_DB_DSN")
    optuna_dsn = os.environ.get("OPTUNA_DB_URL")
    
    db_cfg = PostgresOrchestrationDbConfig(dsn=db_dsn)
    db = PostgresOrchestrationDb(db_cfg)
    
    if args.restart_from_scratch:
        reset_stage_from_scratch(
            db=db,
            stage_no=args.stage,
            model_type=args.model,
            optuna_storage_url=optuna_dsn
        )
    else:
        print("Running dispatcher...")
        run_dispatcher_from_postgres(
            db=db,
            stage_no=args.stage,
            model_type=args.model,
            optuna_storage_url=optuna_dsn
