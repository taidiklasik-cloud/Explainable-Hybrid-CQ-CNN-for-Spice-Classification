...
class RcloneCheckpointConfig:
    ...
    def from_env(cls) -> "RcloneCheckpointConfig":
        ...
        return cls(
            ...
            rclone_executable=os.environ.get("RCLONE_EXE", "rclone"),
        )
...
def checkpoint_file_name(checkpoint_type: str, epoch_number: int | None = None) -> str:
    ...
    if key in {"INTERVAL", "EPOCH"}:
        ...
        return f"interval_epoch_{int(epoch_number):03d}.pt"
    if key == "BEST":
        return "best.pt"
    if key == "FINAL":
        return "final.pt"
...
def build_gdrive_relative_path(
    *,
    stage_no: int,
    model_type: str,
    task_id: int,
...
def upload_checkpoint_via_rclone(
    *,
    local_path: str | Path,
    ...
    if not local.is_file():
        raise FileNotFoundError(f"Checkpoint file not found: {local}")

    remote_target = f"{config.rclone_remote}:{gdrive_relative_path.replace(chr(92), '/')}"
    command = [config.rclone_executable, "copyto", str(local), remote_target]
    if extra_args:
        command.extend(extra_args)

    subprocess.run(command, check=True)
    return checkpoint_metadata(local_path=local, gdrive_relative_path=gdrive_relative_path, config=config)
