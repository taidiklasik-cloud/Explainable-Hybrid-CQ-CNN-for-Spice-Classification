...
def build_runtime_plan(model_type: str, profile: WorkerRuntimeProfile, resources: WorkerResources) -> RuntimePlan:
    """Build a conservative per-model runtime plan from detected worker resources."""
    model_key = normalize_model_type(model_type)
    torch_device = resources.torch_device
    target = max(1, int(profile.target_effective_batch_size))

    if torch_device.startswith("cuda"):
        if model_key == "hybrid":
            base_micro = 4 if resources.cuda_total_vram_gib < 8 else 8
        else:
            base_micro = 16 if resources.cuda_total_vram_gib < 8 else 32
    else:
        base_micro = 2 if model_key == "hybrid" else 8
    ...
    if model_key != "hybrid":
        ...
        fallbacks: Tuple[str, ...] = ()
        notes = "classical model uses torch backend only"
    else:
        quantum_device = str(q["q_device"])
        fallbacks = tuple(q["q_device_fallbacks"])
        notes = "hybrid model uses PennyLane with ordered quantum device fallback"
...
def print_runtime_summary(resources: WorkerResources, plans: List[RuntimePlan]) -> None:
    ...
    for plan in plans:
        print(
            ...
            f"accum={plan.gradient_accumulation_steps}, effective={plan.effective_batch_size}, "
